...
def _async_client_and_model() -> tuple[Any, str]:
    """Async client + model. Azure OpenAI (gpt-5.4-nano) preferred when set;
    OpenAI is the fallback. The deploy has no OpenAI key, so Azure keeps the
    streaming chat alive in production."""
...
async def stream_answer(
    ...
    if not _chat_configured():
        yield {
            "type": "error",
            "message": "Chat agent is not configured (no Azure or OpenAI credentials).",
        }
        return

    client, model = _async_client_and_model()
