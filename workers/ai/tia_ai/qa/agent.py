...
def _client_and_model() -> tuple[Any, str]:
    ...
    never hangs the request. The deploy has no OpenAI key — Azure is the only
    configured provider — so this seam keeps chat working in production."""
    if AZURE_AI_ENDPOINT and AZURE_AI_KEY:
        ...
        return (
            ...
            AZURE_CHAT_MODEL or "gpt-5.4-nano",
        )
...
def _client() -> Any:
    """Back-compat shim: the client only (model is chosen via _client_and_model)."""
    return _client_and_model()[0]
...
def route_intent(text: str) -> str | None:
    ...
    (no regex). Returns "timesheet" | "greeting" | "chat", or None if the model
...
def answer(
    ...
    for _ in range(max_steps):
        ...
        except Exception:  # noqa: BLE001 — never surface a raw LLM/network error
            return {
        ...
        from ..ai.guard import looks_like_blank_refusal
