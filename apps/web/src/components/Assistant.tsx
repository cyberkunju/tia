  ...
  const send = async (text: string) => {
    ...
    try {
      ...
      if (unconfigured) {
        ...
        setMsgs((m) => [...m, { role: "aida", text: r.answer, cites: r.citations }]);
      }
    } catch {
      setMsgs((m) => [...m, { role: "aida", text: localAnswer(t, docs ?? [], invoices ?? []) }]);
    ...
    <AnimatePresence>
      {open && (
        <>
          ...
          <motion.aside
            className="fixed right-0 top-0 bottom-0 w-full max-w-[400px] bg-white border-l border-ink-200 shadow-lg z-50 flex flex-col"
            initial={{ x: 440 }} animate={{ x: 0 }} exit={{ x: 440 }} transition={{ type: "spring", stiffness: 300, damping: 34 }}
          >
            <header className="flex items-center justify-between px-4 h-12 border-b border-ink-200">
              <div className="flex items-center gap-2">
                <span className="grid place-items-center h-7 w-7 rounded-md bg-brand-50 text-brand-600 ring-1 ring-brand-100"><Sparkles size={15} /></span>
                <div className="leading-tight"><div className="text-sm font-semibold text-ink-900">AIDA</div><div className="text-2xs text-ink-400">Context-aware assistant</div></div>
              </div>
